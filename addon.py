...
def INDEX_CHANNELS(cid):
    ...
    try:
        ...
        xbmcgui.Dialog().notification('[ You don\'t have premissions to watch this TV channed ]', 'You probably have to pay additionally in order to watch this TV channel.', xbmcgui.NOTIFICATION_WARNING, 10000, sound=True)
    ...
    if quality == 'moderate':
        ...
        liz.setInfo(type="Video", infoLabels={"Title": play_list[1]})
...
def LIST_REC_CHAN(url):
    ...
    for temp1, day, temp2, rec_url, time, lrc_name in match:
        del temp1, temp2
        if day != '':
            addLink('=['+day+']=', '', '')
...
if not mode or not cid or len(cid) < 1:
